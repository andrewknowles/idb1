...
--
-- Host: 127.0.0.1
-- Generation Time: Mar 17, 2015 at 11:12 AM
...
(1, 'Company Select', 'select cpy_id, cpy_screen from cpy', 'xxxxx', 'xxxxx', 0, 0, 0, 0),
(2, 'Select Branches', 'select bra_id, bra_screen from bra', 'x', 'x', 0, 0, 0, 0),
(3, 'users', 'select usr_id, usr_first, usr_last, usr_authorize from idb_users\r\nwhere usr_login = ? and usr_passwd = ?', 'x', 'x', 0, 0, 0, 0),
...
(86, 'Part Lines Delivered Not Invoiced', 'SELECT odh.odh_doc_id Document, odh_doc_id_seq Sequence, bra.bra_screen Branch, xvd.xvd_display Dept, odh.odh_d_closed D_closed, opl.pmf_id, opl.pro_id, sum(opl_q_del) Q_del, sum(opl_q_inv) Q_inv, round(sum(opl_q_del * opl_cost),2) value \r\nFROM opl, odh, bra, xvd, ofh, oft, ofs \r\nWHERE opl_status = 4 \r\nAND opl_line = 1 \r\nAND ofs.dpr_id = xvd.xcv_id \r\nAND xvd.lng_id = 0 \r\nAND xvd.xcd_id = 1 \r\nAND opl.opl_cancel=0\r\nAND opl.opl_q_all <> 0 \r\nAND odh.odh_cancel=0 \r\nAND odh.bra_id = bra.bra_id \r\nAND opl.odh_id = odh.odh_id\r\nand opl.ofh_id = ofh.ofh_id\r\nand ofh.oft_id = oft.oft_id\r\nand opl.ofh_id = ofs.ofh_id and opl.ofs_id = ofs.ofs_id\r\nand oft.oft_process = ''1'' ', 'AND odh.bra_id = ? GROUP BY 1,2,3,4,5,6,7 ORDER BY 3,4,1,2', 'AND odh.cpy_id = ? GROUP BY 1,2,3,4,5,6,7 ORDER BY 3,4,1,2\r\n   ', 12, 8, 0, 0),
...
(101, 'Labour WIP Summary By Branch/Dept', 'select bra.bra_screen branch, xvd_dpr.xvd_display dept, round(sum((orl_q_passed - orl_q_gain) * ofl.ofl_cost),2) value \r\nfrom ofl, ofs, oft, orl, ofh, bra, xvd xvd_dpr\r\nwhere ofl_line = 3 and ofl_q_cord <> ofl_q_cinv\r\nand ofl.ofs_id = ofs.ofs_id\r\nand ofl.ofh_id = ofs.ofh_id\r\nand ofs.oft_id = oft.oft_id\r\nand ofs.ofs_sqciw in (0,2,3,4)\r\nand ofl.ofh_id = orl.ofh_id and ofl.ofl_id = orl.ofl_id \r\nand ofs_status <> 3\r\nand ofl.ofh_id = ofh.ofh_id\r\nand ofs.bra_id = bra.bra_id\r\nand xvd_dpr.xcd_id = 1 and xvd_dpr.xcv_id = ofs.dpr_id and xvd_dpr.lng_id = 0', ' and ofs.bra_id = ?  group by 1,2 order by 1,2', ' and ofs.cpy_id = ? group by 1,2 order by 1,2', 14, 1, 0, 0),
(102, 'Labour WIP Detail', 'select bra.bra_screen branch, xvd_dpr.xvd_display dept, ofh.ofh_doc_id document, ofs.ofs_id segment, xvd_oft.xvd_display folder,  sum(ofl_q_cord) wo_qty, sum(ofl_q_cinv) inv_qty,\r\n sum(orl_q_passed - orl_q_gain) clock_hours, round(sum((orl_q_passed - orl_q_gain)* ofl.ofl_cost),2) value \r\nfrom ofl, ofs, oft, orl, ofh, bra, xvd xvd_dpr, xvd xvd_oft\r\nwhere ofl_line = 3 and ofl_q_cord <> ofl_q_cinv\r\nand ofl.ofs_id = ofs.ofs_id\r\nand ofl.ofh_id = ofs.ofh_id\r\nand ofs.oft_id = oft.oft_id\r\nand ofs.ofs_sqciw in (0,2,3,4)\r\nand ofl.ofh_id = orl.ofh_id and ofl.ofl_id = orl.ofl_id \r\nand ofs_status <> 3\r\nand ofl.ofh_id = ofh.ofh_id\r\nand ofs.bra_id = bra.bra_id\r\nand xvd_dpr.xcd_id = 1 and xvd_dpr.xcv_id = ofs.dpr_id and xvd_dpr.lng_id = 0\r\nand xvd_oft.xcd_id = 24 and xvd_oft.xcv_id = ofs.oft_id and xvd_oft.lng_id = 0', ' and ofs.bra_id = ? group by 1,2,3,4,5 order by 1, 2', ' and ofs.bra_id = ? group by 1,2,3,4,5 order by 1, 2', 14, 2, 0, 0),
(103, 'Parts WIP Summary', 'SELECT bra.bra_screen Branch, xvd.xvd_display dept, \r\nround (sum(opl_q_del * opl_cost),2) value \r\nFROM opl, odh, bra, xvd, ofh, oft, ofs \r\nWHERE opl_status = 4 \r\nAND opl_line = 1 \r\nAND ofs.dpr_id = xvd.xcv_id \r\nAND xvd.lng_id = 0 \r\nAND xvd.xcd_id = 1 \r\nAND opl.opl_cancel=0\r\nAND opl.opl_q_all <> 0 \r\nAND odh.odh_cancel=0 \r\nAND odh.bra_id = bra.bra_id \r\nAND opl.odh_id = odh.odh_id\r\nand opl.ofh_id = ofh.ofh_id\r\nand ofh.oft_id = oft.oft_id\r\nand opl.ofh_id = ofs.ofh_id and opl.ofs_id = ofs.ofs_id\r\nand oft.oft_process = ''1''', ' AND odh.bra_id = ? GROUP BY 1,2 ORDER BY 1,2 ', ' AND odh.cpy_id = ? GROUP BY 1,2 ORDER BY 1,2 ', 14, 3, 0, 0),
(104, 'Unreversed Accrual Lines', 'select count(*) cnt from gtr, gbt where gtr_accrual = 1 and gtr.gbt_id = gbt.gbt_id', ' and gtr.bra_id = ?', ' and gtr.cpy_id = ?', 31, 5, 0, 105),
(105, 'Unreversed accrual batches', 'select gbt.gbt_doc_id document, gbt.gbt_desc description, \r\ncase when gtr_posted = 0 then ''No'' else ''YES'' end posted,\r\n gbt.bra_id branch, gbt.gll_id ledger, gbt.gpr_id period, gbt.gpr_year year, \r\nsum(gtr_debit) debit, sum(gtr_credit) credit from gtr, gbt where gtr_accrual = 1 and gtr.gbt_id = gbt.gbt_id', ' and gtr.bra_id = ? group by 1,2,3,4,5,6,7\r\norder by 7,6,5,4 ', ' and gtr.cpy_id = ? group by 1,2,3,4,5,6,7\r\norder by 7,6,5,4', 32, 5, 0, 0),
(106, 'xxxxxx', 'x', 'x', 'x', 0, 0, 0, 89),
(107, 'x', 'x', 'x', 'x', 0, 0, 0, 0);
